...
const InstallAppButton: React.FC<Props> = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    setIsIos(/iPhone|iPad|iPod/i.test(navigator.userAgent) && !/Android/i.test(navigator.userAgent));
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white rounded-md p-5 max-w-sm w-full text-center space-y-4">
            ...
            <p className="text-sm text-gray-700">
              On iOS, tap the share button in Safari (square with arrow) then choose <strong>Add to Home Screen</strong> to install the app.
            </p>
            <button
